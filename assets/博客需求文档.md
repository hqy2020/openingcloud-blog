...
**数据来源：** D1 `timeline_nodes` 表（SSR）或静态数据 fallback
...
**数据来源：** D1 `timeline_nodes` 表，筛选 `impact = 'high'` 的节点 + 静态高光成就数据
...
**阅读数来源：**
- D1 `post_views` 表记录每篇文章阅读数
- 构建时从 D1 拉取，或 SSR 实时查询
...
**D1 表：** `timeline_nodes`（已有，需扩展 type 约束）
