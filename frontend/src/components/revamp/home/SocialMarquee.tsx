...
function AchievementMarqueeCard({ card }: { card: AchievementCard }) {
  return (
    <motion.article
      ...
      className="group w-[280px] shrink-0 rounded-2xl border border-slate-200/80 bg-claude-ivory px-5 py-4 shadow-[0_10px_22px_rgba(15,23,42,0.1)] transition-colors duration-200 hover:border-claude-terracotta hover:bg-claude-near-black hover:shadow-whisper-lg"
    >
      <p className="text-[11px] font-semibold tracking-[0.16em] text-slate-500 group-hover:text-claude-coral">{card.period}</p>
      <p className="mt-1.5 text-sm font-semibold text-slate-800 group-hover:text-white">{card.title}</p>
      <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-slate-600 group-hover:text-claude-coral">{card.description}</p>
    </motion.article>
  );
}
